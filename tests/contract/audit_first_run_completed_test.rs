...
fn audit_schema() -> Value {
    ...
    let contents = fs::read_to_string(&path).unwrap_or_else(|error| {
        panic!(
            "failed to read audit schema at {}: {error}",
            path.display()
        );
    });
...
fn audit_schema_requires_event_version_payload() {
    ...
    let required = schema["required"]
        .as_array()
        .expect("schema should define required top-level keys");
...
fn payload_has_source_count_by_kind_with_required_keys() {
    ...
    let obj = counts
        .as_object()
        .expect("source_count_by_kind should be an object");
...
fn payload_source_counts_are_integers() {
    ...
    for kind in ["raw", "calibration", "project", "inbox"] {
        assert!(
            counts[kind].is_u64(),
            "source_count_by_kind.{kind} must be an integer"
        );
    }
}
...
fn payload_raw_and_project_counts_are_at_least_one() {
    ...
    assert!(
        counts["raw"].as_u64().unwrap() >= 1,
        "raw count must be >= 1 per contract"
    );
    assert!(
        counts["project"].as_u64().unwrap() >= 1,
        "project count must be >= 1 per contract"
    );
}
