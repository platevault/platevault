...
| 048 per-frame-inventory | 🟠 in progress (external session + campaign lane D, reconcile-first) | `tasks.md` still shows 0/44 ticked, but real code has landed on `main`: session frame counts + disk usage (`5dc00c90`/#435), raw-frame-vs-disk reconciliation + symlink-gated scans (`a70db417`/#442), an accurate per-frame-type destination-pattern preview (`58ccfcbd`/#390), and a real-UI journey (`c526dc10`/#470). A separate fresh worktree (branch `048-complete-per-frame-inventory`, no commits yet) is independently working this spec concurrently with campaign lane D; lane D reconciles onto whatever that session lands (rebase + fresh `speckit-verify mode:tasks` against real code) before implementing only the verified remainder. This row will move again when lane D reports |
