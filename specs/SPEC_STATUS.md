...
| 044 targets-planner-astronomy | ⚪ Placeholder — Track B spec in progress (user session) | Split 2026-07-04: 044 = **Track B** (observer-location ephemeris engine: altitude curves/rise/set/transit, research-gated, being specced by the user separately); the moon/filter half moved to **047**. Frontend still mocked |
| 045 review-state-real | 🔴 Superseded by 041 | |
| 046 i18n-error-codes | ✅ Implemented | 36/36 (#311–#314) |
| 047 targets-planner-moon-filters | ⚪ Spec drafted (Track A moon/filters); clarify pending | Track A of the planner split: moon phase/illumination/separation + filter guidance + opposition from date/time + target RA/Dec only (no observer location — that boundary is Track B/044). spec.md authored 2026-07-04 on `047-targets-planner-moon-filters`; awaiting clarify answers before plan/tasks. Former blocker #57 (list-endpoint RA/Dec enrichment) resolved |
