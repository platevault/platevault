...
- [x] T048 [P] Contract test for `plan.resume` in `tests/contract/plan_resume.spec.ts` (success, `run.not_paused`, `volume.still.unavailable`, `disk.still.full`, `item.still.stale`). PARTIAL, evidence-backed: `conformance-harness.mjs` fixtures cover the schema shape for `success`, `run.not_paused`, and confirm the reserved re-validation codes are schema-valid. The three re-validation codes (`volume.still.unavailable`/`disk.still.full`/`item.still.stale`) are declared in `plan.resume.json` but NOT YET produced by `resume_plan` — its own docstring says "For v1, we trust the caller to have resolved the condition" (no re-validation implemented). Filed as part of issue #575 (which also covers the more severe finding: `resume_plan` does not re-spawn the executor, so a resumed run never actually continues processing pending items — see T012).
- [ ] T049 [P] Integration test: pause on `volume.unavailable` in `tests/integration/apply_pause_volume.rs`. RE-VERIFIED DEFERRAL (prior note was accurate): `FailureCode::VolumeUnavailable` requires a raw OS error `ENODEV`(19)/`ENXIO`(6) (`crates/fs/executor/src/failure.rs::classify_io_error`), not producible via ordinary tempdir filesystem operations in a portable test. `stale_source_triggers_pause` (executor unit test) remains the closest deterministic coverage of the *pause mechanism*; the volume-specific trigger condition itself is untestable without real device unavailability.
- [x] T050 [P] Integration test: pause on `item.stale` in `tests/integration/apply_pause_stale.rs`. GAP FILLED with a real cross-crate test (this one WAS achievable — the prior "same as T049" note undersold it): `apply_pauses_on_stale_item_cas_mismatch` in `crates/app/core/tests/plan_apply_lifecycle_integration.rs` seeds a real approved item with a mismatched `approved_size_bytes` CAS snapshot, applies through the real executor, and asserts the plan reaches `paused` with `pause_reason = "item.stale"` via `get_apply_status`.
