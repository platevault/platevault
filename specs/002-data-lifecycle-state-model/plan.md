...
**Language/Version**: Rust 1.75+ (workspace toolchain), TypeScript 5.x
(apps/desktop), JSON Schema Draft 2020-12 (`packages/contracts/`)

**Primary Dependencies (Rust)**:
- `sqlx = { version = "*", features = ["sqlite", "runtime-tokio-rustls", "macros", "migrate"] }` — async SQLite persistence
- `tokio = { version = "1", features = ["sync", "rt-multi-thread", "macros"] }` — async runtime + in-process event-bus broadcast
- `schemars` — JSON Schema generation from Rust DTOs (Rust is canonical source; JSON files are reproducible projections per research.md §9)
- `serde`, `thiserror`, `time`, `uuid` — already in workspace deps
- `specta` + `tauri-specta` — TypeScript codegen for Tauri command signatures

**Storage**: SQLite via sqlx. Single canonical local database. Migrations as
inline `*.sql` files consumed by `sqlx::migrate!()`. Generated manifests
remain projections; the database is the durable record.

**Testing**: `cargo test --workspace` for unit + integration; JSON-Schema
fixture tests for contracts (Vitest); fixture tests in
`crates/contracts/core/tests/` verify Rust DTOs round-trip through their
generated JSON schemas; Playwright MCP smoke for UI wiring after Rust port
lands.
...
**Event bus**: Hybrid. In-memory via `tokio::sync::broadcast::Sender<EventEnvelope>`
for live UI delivery; durable replay via a SQLite `events` table that
subscribers can read with a `since: event_id` cursor (research.md §6.1). The
`source: 'restore'` envelope field (R-Source-1) discriminates replay events
from live events at subscribers. This spec owns the canonical event-bus design
note for the whole project (per GRILL 2026-05-21 cross-cutting decision).
Publication is transactional with the entity mutation + audit write; refused
transitions and no-ops do not publish. Subscribers include the
projection-staleness recomputer (FR-003), spec 010's guided-flow triggers,
spec 019's log panel, and spec 024's manifest regenerator.

**Contract source**: Rust-canonical. Rust DTOs in `crates/contracts/core/`
(with `#[derive(JsonSchema, Serialize, Deserialize)]`) are the canonical
source of truth; `specs/<NNN>/contracts/*.json` are reproducible projections
generated by `cargo run --bin generate-contracts` (see research.md §9).

### Build/codegen pipeline

- `cargo run --bin generate-contracts` — regenerates all `specs/*/contracts/*.json`
  from Rust DTOs via `schemars`. CI runs this step and asserts
  `git diff --exit-code specs/*/contracts/*.json`; a non-zero diff fails the
  build.
- `cargo run --bin generate-bindings` (or equivalent `tauri-specta` hook) —
  regenerates `apps/desktop/src/bindings/*.ts` from Rust command signatures
  via `specta` + `tauri-specta`. CI asserts `git diff --exit-code apps/desktop/src/bindings/`
  after regeneration.
