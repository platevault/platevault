...
- [ ] T008 ~~Add fresh-DB reset to `tauri-app.ts`~~ **SUPERSEDED** — fresh-DB reset in the thirtyfour harness is `reset_database()` in `crates/e2e-tests/tests/common/mod.rs` (reads `PV_DB_URL` env; TODO wiring for OS app-data path). `tauri-app.ts` is now a reference scaffold.
...
> `PV_DB_URL` (previously unset, so `reset_database()` silently no-opped).
...
- [X] T028 [US3] `e2e.yml` rewritten to uniform 3-OS matrix (ubuntu/windows/macos) via `tauri-plugin-webdriver` (D10). Drops old `tauri-driver`/msedgedriver/WebKitWebDriver steps. Per OS: install `tauri-webdriver` CLI, build frontend with `VITE_E2E=1`, serve dist on :5173, build `desktop_shell --features e2e`, run nextest (`xvfb-run -a` on Linux). Needs gate from `integration` job (ci.yml Stage A). 2026-07-04: added `PV_DB_URL` (fresh file-backed DB per run) now that journeys are real.
