...
- [ ] T134 Tests: real 0 renders as "0" with source pill; missing numeric renders unresolved chip (never 0, no source pill); n/a renders blank/"—" without chip; contract round-trips null; each detail panel adds ≥1 non-row information class (SC-010, SC-011); regression test pinning calibration matching results unchanged by de-zeroing (matching runs on Option-typed `SessionInfo`/`MasterInfo`, never the contract DTO — pin that with a fixture with absent gain/exposure)
