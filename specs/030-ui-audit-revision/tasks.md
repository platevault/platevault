...
- [ ] T132 Adopt the shared renderer across all metadata surfaces: Inbox review, Sessions detail, Calibration (incl. `MastersTable` meta lines/cells), Targets, Projects, Archive; per-field applicability read from the data-model.md matrix; composed identifying strings omit missing tokens (FR-137). **Partial**: Calibration (MasterDetail + MastersTable, per-kind applicability) and Sessions detail (all fields — Light column is all-✓) are done. Inbox review, Targets, Projects, Archive detail panels still conditionally *omit* rows for absent values (collapsing missing into not-applicable) rather than passing them through PropertyTable's default `applicable` marker — not reworked; scope cut for effort budget, flagged for follow-up.
- [ ] T133 Detail-as-delta rework: audit every detail panel against its list row; lead with new information (full metadata, provenance, related entities, history, actions); trim echoed columns to a small identifying summary; keep panels curated (FR-139, FR-140). **Partial**: reviewed MasterDetail only — already compliant (used-by/compatible/match-panel/actions beyond the row; fingerprint table reads as the permitted "small identifying summary"). Inbox/Sessions/Targets/Projects/Archive detail panels not audited.
- [x] T134 Tests: real 0 renders as "0" with source pill; missing numeric renders unresolved chip (never 0, no source pill); n/a renders blank/"—" without chip; contract round-trips null; regression test pinning calibration matching results unchanged by de-zeroing (matching runs on Option-typed `SessionInfo`/`MasterInfo`, never the contract DTO — pinned with a fixture with absent gain/exposure). **Not covered**: an exhaustive "each detail panel adds ≥1 non-row information class" (SC-011) assertion across every surface — only reviewed for MasterDetail, not test-asserted per-surface.
