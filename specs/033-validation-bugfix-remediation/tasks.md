...
- [X] T005 Verify deps resolve and gates stay green: `cargo build --workspace`, `pnpm install`, `just lint`, `just typecheck`. Done (2026-07-11): all four green — `cargo build --workspace` (23 crates), `pnpm install`, `just lint` (0 errors), `just typecheck` (0 errors).
...
- [X] T085 [P] Update docs: mark resolved items in `docs/development/autonomous-run-2026-06-validation-findings.md` "Remaining"; refresh `PRODUCT.md`/native-dev docs as needed. Done (2026-07-11): every "Remaining" bullet (025 T1-2, 016 T1-1, 009 T1-3, 006/007/023, 014 T1-4, 024/012/010) annotated **RESOLVED** with the closing spec-033 task IDs. `PRODUCT.md` checked — no stale WSL/real-backend-harness claims there, no change needed. `docs/development/test-strategy-033.md` (native-dev-adjacent test-layer doc) got a 2026-07-11 update note: its **RB** layer (`apps/desktop/e2e/real-backend/*.spec.ts`) was superseded by spec 037's `crates/e2e-tests` harness; the dead `apps/desktop/e2e/README.md` + `tsconfig.json` scaffold it pointed to (referencing a directory structure that no longer exists — the actual source of my T015/T024/T031 investigation confusion) was removed. Left `docs/development/ci/ci-033.proposed.yml` untouched: it's an explicitly "proposed" (never-adopted) historical design artifact — the real CI is `.github/workflows/e2e.yml`, a different, already-shipped design.
- [~] T086 Run the full gate suite + `quickstart.md` core-journey validation on the real backend (SC-001/SC-008). Gate suite: green (2026-07-11) — `just lint` (0 errors), `just typecheck` (0 errors), `cargo nextest run --workspace` (2025 passed, 23 skipped — pre-existing `#[ignore]` Layer-2 gates), `bash scripts/check-db-boundary.sh` (0 production query sites outside `crates/persistence/db`), `pnpm test` (142 files / 1340 vitest tests passed), `cargo nextest run -p e2e_tests --no-run` (compiles clean). Real-backend core-journey (SC-001): quickstart.md's manual Layer-3 (WSL xvfb) / Layer-4 (Windows-native, human-driven) procedure is superseded for automated-evidence purposes by spec 037's `crates/e2e-tests` Layer-2 harness, which exercises every SC-001 journey beat against the real `desktop_shell` binary + real SQLite in CI on Linux+Windows: ingest→sessions (`ingestion_sessions_search`), split mixed folder (`inbox_ui_journeys.rs::inbox_ui_mixed_folder_splits_into_single_type_items`), calibration suggestions (`calibration_ui_journeys.rs`), create project (`journeys.rs::first_run_resolve_create_project`, `lifecycle_ui_journeys.rs`), generate+apply plan audited (`plan_review_apply_with_audit`), manifests persist (`journeys.rs` + `workflow_run_manifest_e2e.rs`), protected-source cleanup block (`cleanup_plan_review`), Cmd+K target search (`targets_journeys.rs`). Latest green run on `main` itself: `gh run` 29146855462 (2026-07-11T08:54Z), "Real-UI E2E" workflow, ubuntu-latest + windows-latest both success. NOT marked `[X]`: this pass did not personally drive Layer 3/4 (forbidden by run coordination — a concurrent agent was executing journeys against the same ports/single-instance app) and the SC-001 phrasing ("a new user can complete the journey... zero crashes") implies the human-facing Layer-4 interactive pass, which is T081's runbook artifact's job, not this task's automated-evidence half. The automatable half (gates + real-backend CI proof) is done; the literal interactive walkthrough remains for whoever next runs the Windows-native Layer-4 pass.
- [X] T087 [P] Remove dead code left by the removed deps/stubs (react-table, md-editor, trash stub, search fixtures). Done (2026-07-11), four items adjudicated with evidence:
      (a) `@tanstack/react-table`: REMOVED from `apps/desktop/package.json` (`pnpm install` to sync lockfile). It was already removed once by this spec's T003 but got reinstalled later for spec-044 T036's planned `TargetsTable` wiring, which spec 044 explicitly DEFERRED ("net-zero dep", `specs/044-targets-planner-astronomy/tasks.md:464-475`) — leaving it with zero current imports (confirmed via `grep` across `apps/desktop/src` and `knip --dependencies`, which flagged only this package, not the genuinely-used `@tanstack/react-virtual`). Re-adding one leaf npm dependency is trivial if/when that refactor is picked up (YAGNI); `just typecheck`/`pnpm exec vitest` stayed green after removal.
      (b) md-editor + trash stub: already fully clean — `@uiw/react-md-editor` has no `package.json` entry and no source references (removed by T003 already); the "trash stub" (`trash_returns_unavailable_in_v1`) is fully replaced by `crates/fs/executor/src/ops/trash_op.rs`'s real `trash_moves_file_to_os_bin_or_uses_fallback` test (T014) — the only remaining references are doc-comments citing the old name for history. No action needed.
      (c) `angularSeparationFromMoonDeg` (`apps/desktop/src/features/targets/planner-astronomy.ts`): KEPT, not deleted — confirmed zero production callers (only `astro/lunar-separation.ts`'s geocentric `lunarSeparationDeg` feeds real UI: `TargetsTable`/`GuidanceCell`/`TargetDetailV2`, per the module's own SC-013 "no duplicate Moon-geometry computation" note). Marked explicitly test-only in its doc comment: it anchors `planner-astronomy.test.ts`'s Horizons-anchored (JPL astrometric RA/Dec) topocentric-vs-geocentric tolerance cross-check, independent of and complementary to the adjacent test that validates the actual production geocentric formula against the same external reference. Deleting it would lose the external-ephemeris tolerance proof; refactoring the test to the production formula already exists as a separate test right next to it.
      (d) `plans_review_retry_failed`/`plans_review_retry_no_items` (`apps/desktop/messages/en.json`): DROPPED, not wired. Root cause: `PlanReviewOverlay.tsx`'s three catch blocks (`handleApproveAndApply`, `handleDiscard`, `handleGenerateRetryPlan`) used the `e instanceof Error ? e.message : String(e)` anti-pattern (renders `"[object Object]"` for a thrown `ContractError`) instead of the existing spec-046 canonical `errMessage()` helper (`@/lib/errors`) — a real, separate bug, now fixed in all three. `errMessage()` resolves `plans.retry`'s `NoItemsToRetry`/`no.items.to.retry` code through the EXHAUSTIVE `ERROR_MESSAGES` catalog (`err_no_items_to_retry`, already registered) — wiring the two dedicated keys instead would have created a second, inconsistent translation path and a duplicated catalog value (the same anti-pattern `error-messages.ts` explicitly calls out avoiding for other codes, e.g. `internal.error`/`canonical_target.not_found`). `pnpm i18n:compile` + `pnpm exec vitest run src/features/plans/PlanReviewOverlay.test.tsx` (7/7) confirm the fix.
