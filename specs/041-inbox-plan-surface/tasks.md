...
- [X] T071 [US10] confirm.rs: delete split/mixed branch; one rootId/item; retire per-type grouping (FR-050). Verified: `crates/app/inbox/src/confirm.rs:179-194` enforces single-type-only confirmation (no `action` field; only `classified` results confirmable).
- [X] T072 [US13] Contracts + binding regen: inbox.list (groupId/groupKey/groupLabel/frameType/sourceGroup/missingMandatory), inbox.confirm (drop action), inbox.reclassify (property map+bulk), metadata DTO new fields (FR-043/FR-044/FR-050). Verified: `crates/contracts/core/src/inbox.rs:370-398` carries the single-type sub-item identity fields (group_id/group_key/group_label/source_group_id/frame_type); bindings regenerated.
- [X] T073 [P] [US10] Layer-1 + vitest tests for Phase 12. Verified: `cargo test -p app_core_inbox` — 138 passed, 0 failed.
...
- [X] T076 [US14] Drop session review lifecycle (states + Confirm/Re-open/Reject); sessions derived; editable metadata view (FR-051). Verified: `crates/persistence/db/migrations/0050_session_lifecycle_drop.sql` + `apps/desktop/src/features/sessions/SessionsPage.tsx:16-20` (review-state filter and Confirm/Re-open/Reject actions removed; Reveal retained).
- [X] T077 [US14] Migration handling for plan_open legacy items (FR-054). Verified: `crates/persistence/db/migrations/0049_inbox_single_type.sql` (unconditional `sg-migrate-*` source-group backfill) + `crates/app/inbox/src/classify.rs:329` guard (`plan_open` items are not re-split); 2 dedicated tests.
- [X] T078 [US14] `/speckit.sync.conflicts` vs 045/006/035; mark spec 045 superseded. sync.conflicts run recorded 2026-07-09 → sync-conflicts-2026-07-09.md; outcome: markers consistent, 1 live conflict (041 FR-051/SC-018 vs 006 FR-010) reconciled via dated amendment note.
