...
| S3 | Inbox = permanent list-left/detail-right split, absorbs #553 | **Not built, and now withdrawn.** #1068 was decided against building this: #553 (the driver) was already fixed a different way by PR #939, and the owner chose to keep the adaptive dock and the Format column `main` shipped instead | `InboxPage.tsx` passes no `detailPlacement`; `InboxList.tsx:41,138,193-194,408-414,507` shows a shipped Format column instead |
| S4 | Targets side dock at ≥1500px; pinned star+designation; permanent column order; conditional h-scroll | **Not built.** No per-page threshold differentiation shipped (all pages use the 1400px default); no pinned-column or h-scroll work found in `TargetsTable`/`TargetsPage` — confirmed zero `sticky` rules in `TargetsTable.tsx` | `TargetsPage.tsx:543-603` passes no `adaptiveThreshold` |
| S5 | Scroll containment fixed at the container level, absorbs #816 | **Built, now for every consumer.** #1035 fixed #816 for the Target detail specifically; PR #1072 migrated Archive/Projects/Targets onto `DetailPanel` too, closing #1067, so the container guarantee now applies to all six adopters | commit `fa863492`, `DetailPanel.tsx`, `DetailPanel.shared-guard.test.ts` |
| S6 | Migrate `TargetDetailV2` to shared `DetailPanel` | **Built.** PR #1072, closing #1067. `TargetDetailV2.tsx:941` renders `<DetailPanel fill ...>` — the grep below now DOES include `features/targets/` | `grep -rln DetailPanel apps/desktop/src` now includes `features/targets/TargetDetailV2.tsx` |
...
| D5 | Fully-shared components: migrate `TargetDetailV2` **and** Archive to `DetailPanel`; delete the dead `'side-and-bottom'` path | **Migration built** for Archive/Targets/Projects via PR #1072, closing #1067. The `'side-and-bottom'` path was **not deleted** — it remains a live, tested `ListPageLayout` capability, just currently unused by any page (see plan.md's note on `ProjectsPage.tsx`'s stale docstring, tracked as #1108) | `ListPageLayout.tsx:92,207-267`, `ListPageLayout.test.tsx:102-176` |
...
bug, unlike #1066 (the Auto-return bug, fixed by PR #1070) and #1067 (the
DetailPanel adoption gap, closed by PR #1072), which were reported and have
since shipped fixes.
