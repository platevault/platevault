 ...
 *
 * Migrated to Panda CSS (pilot/css-panda branch).
 */
...
export function LogEntryRow({
  ...
  // #626: a link is only "linkable" when a real route exists for it.
  const hasEntityLink =
    hasEntity &&
  ...
  return (
    // eslint-disable-next-line jsx-a11y/no-noninteractive-element-interactions -- interactivity is conditional
    <li
      ref={measureRef}
      data-index={index}
      // eslint-disable-next-line no-restricted-syntax -- dynamic: virtualizer row style passthrough
      style={style}
      className={cx(event, isClickable && eventLink)}
