...
const EXAMPLE_PATHS = [
  '/astro/calibration/masters',
  'D:\\Astrophotography\\Masters',
  '/Volumes/AstroData/Calibration',
];

/**
 * Step — Calibration sources (optional).
 * Where master calibration frames are stored.
 */
export function StepCalibration({ entries, onAdd, onRemove, onScanDepthChange, errors }: StepCalibrationProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--alm-space-5)' }}>
      <Box>
        ...
        >
          ...
          <Pill label="OPTIONAL" variant="ghost" size="sm" />
        </div>
        ...
        >
          Where your master calibration frames (darks, flats, biases) are stored.
...
function AddFolderButton({ onAdd }: { onAdd: (path: string) => void }) {
  ...
  const handleChoose = async () => {
    const result = await pick(undefined, 'calibration');
    if (result.path) {
      onAdd(result.path);
    }
  };
