...
import { bandViability, DEFAULT_MOON_AVOIDANCE, BANDS } from './astro/moon-avoidance';
...
describe('FilterBadges — per-band viability pills (spec 047 T016)', () => {
  it('renders all seven bands with a viable/not-viable aria-label each', () => {
    ...
    render(<FilterBadges viability={viability} recommendation="narrowband-only" />);
    for (const band of BANDS) {
      const state = viability[band] ? 'viable' : 'not viable';
      expect(screen.getByLabelText(`${band}: ${state} tonight`)).toBeInTheDocument();
    }
  });
  ...
  it('renders a single unknown pill instead of fabricating per-band viability', () => {
    ...
    expect(screen.getByText(recommendationLabel('unknown'))).toBeInTheDocument();
    // No band letters rendered as separate pills.
    for (const band of BANDS) {
      expect(screen.queryByLabelText(new RegExp(`^${band}:`))).not.toBeInTheDocument();
    }
  });
