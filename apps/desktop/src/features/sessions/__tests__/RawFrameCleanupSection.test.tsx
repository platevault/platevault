...
vi.mock('@/features/inventory/store', () => ({
  ...
  useGenerateRawFrameCleanupPlan: () => ({ ...generateState, mutate: mockGenerateMutate }),
}));

vi.mock('@/features/plans/PlanReviewOverlay', () => ({
  PlanReviewOverlay: ({ planId, open }: { planId: string | null; open: boolean }) =>
    open ? <div data-testid="raw-cleanup-review-overlay-stub">{planId}</div> : null,
}));
...
describe('RawFrameCleanupSection (spec 048 US3)', () => {
  it('renders the section and calls the raw scan on demand (no fabricated candidates)', () => {
    ...
    expect(screen.queryByTestId('raw-cleanup-candidate-frame-1')).not.toBeInTheDocument();
  ...
  it('renders scanned candidates; protected candidates carry no checkbox', () => {
    ...
    expect(within(row1).getByTestId('raw-cleanup-select-frame-1')).toBeInTheDocument();

    const row2 = screen.getByTestId('raw-cleanup-candidate-frame-2');
    expect(within(row2).queryByTestId('raw-cleanup-select-frame-2')).not.toBeInTheDocument();
  ...
  it('generates a plan with the selected (non-protected) frame ids and opens the review overlay', () => {
    ...
    mockScanMutate.mockImplementation(
      (_vars: unknown, opts?: { onSuccess?: (r: RawFrameCleanupScanResponse) => void }) => {
    ...
    mockGenerateMutate.mockImplementation(
      (
        _vars: unknown,
        opts?: { onSuccess?: (r: { planId: string; itemCount: number }) => void },
      ) => {
    ...
    expect(screen.getByTestId('raw-cleanup-review-overlay-stub')).toHaveTextContent('plan-raw-1');
  });
  ...
  it('renders the no-candidates teaching state for an empty scan result', () => {
    ...
    render(<RawFrameCleanupSection sessionId="session-1" />);
    expect(screen.getByText('No raw sub-frame cleanup candidates')).toBeInTheDocument();
    expect(screen.queryByTestId('raw-cleanup-generate-btn')).not.toBeInTheDocument();
  });
});
