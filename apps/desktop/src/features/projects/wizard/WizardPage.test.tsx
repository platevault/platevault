...
describe('T078c: target detail "new project" button navigates to wizard', () => {
  it('target new-project button is present with correct data-testid', async () => {
    // We test that TargetDetailV2 renders the new-project button.
    // Full navigation test would require router context — we verify the button
    // existence here and rely on the integration test for navigation.
    vi.mock('@/api/commands', async (importOriginal) => {
      const actual = await importOriginal<typeof import('@/api/commands')>();
      return {
        ...actual,
        getTargetIdentity: vi.fn().mockResolvedValue({
          target: {
            id: 'target-001',
        ...
        addTargetAlias: vi.fn().mockResolvedValue({}),
        removeTargetAlias: vi.fn().mockResolvedValue({}),
        renameTargetPrimary: vi.fn().mockResolvedValue({}),
      };
    });

    const { TargetDetailV2 } = await import('@/features/targets/TargetDetailV2');
    const { findByTestId } = render(<TargetDetailV2 targetId="target-001" />);

    const btn = await findByTestId('target-new-project-btn');
    expect(btn).toBeInTheDocument();
    expect(btn).toHaveTextContent('+ New project');
  });
});
