...
export interface ProjectDetailPaneProps {
  projectId: string;
  /** Callback to provide loaded project data to the parent (for the inspector). */
  onProjectLoaded?: (project: ProjectDetailType) => void;
}

export function ProjectDetailPane({ projectId, onProjectLoaded }: ProjectDetailPaneProps) {
  ...
  const currentView: ViewMode = viewModes[projectId] || defaultView;

  function setView(mode: ViewMode) {
    setViewModes({ ...viewModes, [projectId]: mode });
  }

  // Notify parent when project data loads
