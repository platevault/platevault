...
| Keeping the two checkouts in sync | Treat WSL as canonical: commit there and `git pull` in `C:\dev\astro-plan` (its `origin` is GitHub), or re-copy specific uncommitted files. Avoid committing from both trees. |
