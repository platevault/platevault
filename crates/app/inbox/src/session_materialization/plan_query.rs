...
//! child tables. Used by [`super::apply`] to build the per-session work list.
...
/// Row from `inbox_materialization_plan_result_snapshot` (spec 062 §Ingestion).
...
pub struct ProposedSessionRow {
    ...
    pub kind: String,
    /// The `acquisition_site_resolution_revision` row_id pinned by this partition.
...
pub struct SiteResolutionRevisionRow {
    ...
    /// The canonical UTC exposure instant (UTC RFC3339), if known.
    pub canonical_exposure_at_utc: Option<String>,
    /// The derived local noon-to-noon observing-night date (`YYYY-MM-DD`).
    pub observing_night_date: Option<String>,
}

/// Frame row ID from `inbox_plan_result_proposed_session_frame`.
#[derive(Debug, Clone, sqlx::FromRow)]
...
/// Look up the inbox_ingestion_operation subtype row and return the linked
/// `inbox_materialization_plan_result_snapshot` row.
///
...
/// Return frame row_ids for one proposed session, ordered by ordinal.
///
/// # Errors
///
/// Returns [`DbError::Database`] on SQL errors.
...
/// Resolve the observing-night derivation from an acquisition site resolution
/// revision. Returns the revision row with its derived_observing_night field.
///
/// # Errors
///
...
/// Return the `spec062_config_revision.row_id` that the plan snapshot was
/// generated with. This is the `config_revision_row_id` the materialization
/// operation must use.
#[must_use]
pub fn snapshot_config_revision(snapshot: &PlanResultSnapshotRow) -> i64 {
    snapshot.config_revision_row_id
}
