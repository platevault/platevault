...
//! pairs share the same [`protection::generate_cleanup_plan`] tail (PR #408
//! overlap guard, `.astro-plan-archive/<planId>/` destination).
//!
...
/// [`protection::generate_cleanup_plan`] tail as the project-scoped
/// [`generate`] above, so it inherits the PR #408 cross-plan overlap guard
/// and the `.astro-plan-archive/<planId>/` destination for free. Every
...
mod tests {
    ...
    async fn scan_raw_frames_by_session_returns_present_candidates_with_reclaimable_bytes() {
        ...
        let f2 = upsert_frame_record(db.pool(), "root-1", "l2.fits", 2000, "t0", "classified")
            .await
            .unwrap();
        let missing = upsert_frame_record(db.pool(), "root-1", "l3.fits", 500, "t0", "missing")
            .await
            .unwrap();
    ...
    async fn scan_raw_frames_excludes_protected_state() {
        ...
        insert_root(db.pool(), "root-1", "/tmp/lib").await;
        let protected =
            upsert_frame_record(db.pool(), "root-1", "p1.fits", 1000, "t0", "protected")
                .await
                .unwrap();
    ...
    async fn generate_raw_frame_plan_skips_missing_selected_frames() {
        ...
        let present = upsert_frame_record(db.pool(), "root-1", "l1.fits", 1000, "t0", "classified")
            .await
            .unwrap();
        let missing = upsert_frame_record(db.pool(), "root-1", "l2.fits", 2000, "t0", "missing")
            .await
            .unwrap();
